...
function rankBg(rank) {
  ...
  if (rank.topPct <= 5) return 'rgba(39,174,96,0.10)';
    ...
    <button className="grid-row" style={{ '--row-color': p.color, ...style }} onClick={() => onSelect(p)}>
      <span className="grid-cell course">
        ...
        <span className="course-meta">
          {p.country && <img src={countryFlag(p.country)} alt="" className="course-flag" />}
      ...
      <span className="grid-cell diff">
        <span className={`diff-badge ${p.diffVsRun === 'More' ? 'hard' : p.diffVsRun === 'Less' ? 'easy' : 'moderate'}`}>
          <span className="diff-val">{p.diffVsRun === 'Less' ? `\u2212${p.diffVsRunPct.toFixed(0)}%` : p.diffVsRun === 'More' ? `+${p.diffVsRunPct.toFixed(0)}%` : '\u223C'}</span>
          <span className="diff-lbl">{p.diffVsRun === 'Less' ? 'easier' : p.diffVsRun === 'More' ? 'harder' : 'similar'}</span>
        </span>
      </span>
  ...
  return (
    <div className="section projections-section">
      ...
      <div className="segment-tabs">
        {SEGMENTS.map(s => (
          <button key={s.key} className={`segment-tab ${segment === s.key ? 'active' : ''}`} onClick={() => setSegment(s.key)}>
      ...
      {sorted.length === 0 ? (
        <p className="text-muted" style={{ padding: '24px 0', textAlign: 'center' }}>No courses in this category.</p>
      ) : (
        <div className="grid-wrap">
          <div className="grid-hd">
            <span className="gc course">Course</span>
            <span className="gc dist">Dist</span>
            <span className="gc time">Projected</span>
            <span className="gc pace">Pace</span>
            <span className="gc rank">Your Rank</span>
            <span className="gc diff">Difficulty</span>
            <span className="gc adj">vs Pace</span>
            <span className="gc winner">Winners</span>
          </div>
